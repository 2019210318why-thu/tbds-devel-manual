...
套件kafka基于社区的1.0.0版本开发。主要扩展了认证授权，认证的修改包括：
1）增加自定义TBDS认证
...
### 特别注意事项
如果SASL_PLAIN访问方式的机器不在TBDS集群内，需要在TBDS集群上开启白名单模式，具体操作方法如下：
SASL连接kafka需要配置白名单，否则无法通过SASL连接，白名单配置项是`kafka.authentication.saslplain.hosts.whitelist`，默认是无法修改配置的，可以通过编辑启动脚本添加额外的IP。
登录portal机器，修改文件vim /var/lib/tbds-server/resources/common-services/KAFKA/1.0.0/package/scripts/kafka.py，在41行添加

```python
kafka_server_config['kafka.authentication.saslplain.hosts.whitelist'] += u',x.x.x.x,y.y.y.y'
```

添加完重启tbds-server restart，再重启kafka服务即可。
SASL_PLAINTEXT社区认证的端口是6668
TBDS认证的端口是6667
...
1）通过yum源或rpm安装套件版本的kafka到集群外机器：
  ...
  或
    rpm -ivh kafka_2_2_0_0_2041-1.0.0-xxxx.xxxx.x86_64.rpm
...
1）TBDS版kafka jar命名规则

套件的kafka是基于社区二次开发命名规则采用"社区版本号-TBDS-套件版本号"的方式命名.例：我们现在基于社区1.0.0版本的kafka进行开发，套件版本是5.1.3.0，则我们打出的kafka jar版本为1.0.0-TBDS-5.1.3.0，完整的kafka client maven jar文件名为：kafka-clients-1.0.0-TBDS-5.1.3.0.jar

2）基于TBDS提供的maven库开发

（1）拷贝或部署套件提供的maven库到开发者可访问的本地仓库或远程仓库
（2）在客户端maven工程pom引入对应的套件版kafka依赖，以套件5.1.3.0版本为例， 需要在pom中加入的依赖片段（其他版本依次类推）。
...
```text
kafka_2.11-1.0.0-TBDS-5.1.3.0.jar,
kafka-clients-1.0.0-TBDS-5.1.3.0.jar
```

## Kafka Producer开发示例Demo

### jar包

> [https://github.com/2019210318why-thu/TBDS-jar/blob/main/jar_new/New_Kafka_Producer.jar](https://github.com/2019210318why-thu/TBDS-jar/blob/main/jar_new/New_Kafka_Producer.jar)
...
## Kafka Consumer开发示例Demo

### jar包

> [https://github.com/2019210318why-thu/TBDS-jar/blob/main/jar_new/KafkaCustomeDemo.jar](https://github.com/2019210318why-thu/TBDS-jar/blob/main/jar_new/KafkaCustomeDemo.jar)
